...
| TC-1203 | `npm run quality` with local `TEST_DATABASE_URL`/`DATABASE_URL` after locked installs | Lint/type-check, 122 tests, Web/API/Worker builds, and architecture checks passed after final record completion | PASS |
...
| TC-1207 | `npm test`; `npm run validate` | 122/122 tests passed; 22/22 architecture/content checks passed | PASS |
...
3. The first full quality run reached 122 passing tests but Web build failed because the new worktree
