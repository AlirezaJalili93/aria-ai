...
def _publish(args: argparse.Namespace) -> int:
    ...
    result = durable_probe.apply_async(args=[probe_id, args.sleep_seconds])
    print(json.dumps({"probe_id": probe_id, "task_id": result.id}, sort_keys=True))
    return 0
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Celery durable Queue candidate probe")
