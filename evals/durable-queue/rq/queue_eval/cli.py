...
def _publish(args: argparse.Namespace) -> int:
    ...
    arguments = (probe_id, args.sleep_seconds, args.fail_first_attempt)
    candidate_queue = queue()
    if args.delay_seconds > 0:
        job = candidate_queue.enqueue_in(
            timedelta(seconds=args.delay_seconds),
            durable_probe,
            *arguments,
            **options,
        )
    else:
        job = candidate_queue.enqueue(durable_probe, *arguments, **options)
    print(json.dumps({"probe_id": probe_id, "job_id": job.id}, sort_keys=True))
    return 0
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="RQ durable Queue candidate probe")
    commands = parser.add_subparsers(dest="command", required=True)

    publish = commands.add_parser("publish")
    publish.add_argument("--probe-id", required=True)
    publish.add_argument("--sleep-seconds", type=float, default=0.0)
    publish.add_argument("--delay-seconds", type=int, default=0)
    publish.add_argument("--fail-first-attempt", action="store_true")
    ...
    wait.add_argument("--timeout-seconds", type=float, default=90.0)
