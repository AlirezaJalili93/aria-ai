...
function Confirm-LastExit([string]$step) {
    if ($LASTEXITCODE -ne 0) {
        throw "Dramatiq evaluation failed during: $step"
    }
}
...
try {
    ...
    Write-Output 'candidate=dramatiq probe=worker-absent phase=publish'
    ...
    Write-Output 'candidate=dramatiq probe=forced-worker-loss phase=publish'
    ...
    Write-Output 'candidate=dramatiq probe=duplicate-delivery phase=publish-twice'
    ...
    Write-Output 'candidate=dramatiq probe=delayed-delivery phase=publish'
    docker compose run --rm probe publish --probe-id delayed-delivery --delay-ms 3000
    ...
    Write-Output 'DRAMATIQ_EVALUATION_RESULT=PASS'
}
finally {
